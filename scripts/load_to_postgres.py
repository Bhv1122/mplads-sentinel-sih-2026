...
class PostgresDataLoader:
    ...
    def execute_pipeline(self) -> bool:
        ...
        try:
            ...
            # Synchronize BIGSERIAL sequences to max values
            with conn.cursor() as cur:
                for seq_tbl, seq_col, seq_name in [
                    ("financials", "financial_id", "financials_financial_id_seq"),
                    ("progress", "progress_id", "progress_progress_id_seq"),
                    ("risk_scores", "risk_score_id", "risk_scores_risk_score_id_seq"),
                    ("risk_factors", "factor_id", "risk_factors_factor_id_seq"),
                    ("project_history", "history_id", "project_history_history_id_seq"),
                ]:
                    try:
                        cur.execute(f"SELECT setval('{seq_name}', COALESCE((SELECT MAX({seq_col}) FROM {seq_tbl}), 1));")
                    except Exception as seq_err:
                        logger.warning(f"Could not synchronize sequence {seq_name}: {seq_err}")
